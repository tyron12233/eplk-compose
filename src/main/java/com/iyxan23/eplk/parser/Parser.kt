...
import com.tyron.compose.com.iyxan23.eplk.nodes.variable.FieldAccessNode
...
class Parser(private val tokens: ArrayList<Token>) {
    ...
    private fun expression(): ParseResult {
        ...
        if (currentToken.token == Tokens.VAR) {
            ...
            if (currentToken.token == Tokens.DOT) {
                advance()

                val accessResult = result.register(expression()) as Node?
                if (result.hasError) return result

                if (accessResult is VarAccessNode || accessResult is VarAssignNode || accessResult is FunctionCallNode) {
                    // field access node
                    return result.success(
                        FieldAccessNode(
                            identifierToken.value!!,
                ...
                return result.success(accessResult as Node)
            }
